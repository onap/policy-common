...
/*
 * Self-signed keystore, valid for one day (see the argument to the "-valid" flag below).  Used to generate
 * a keystore for use in junit tests.
 */
public class SelfSignedKeyStore {
    ...
    @Getter
    private final String keystoreName = System.getProperty("user.dir") + "/target/test-classes/policy-keystore";
    ...
    public SelfSignedKeyStore() throws IOException, InterruptedException {
        ...
        if (keystore.exists() && System.currentTimeMillis() < keystore.lastModified()
         ...
         */
        String subAltNames = ResourceUtils.getResourceAsString(KEYSTORE_SAN);
        if (subAltNames == null) {
            throw new FileNotFoundException(KEYSTORE_SAN);
        }
