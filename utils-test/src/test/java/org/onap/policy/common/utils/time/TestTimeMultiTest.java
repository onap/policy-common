...
public class TestTimeMultiTest {
    ...
    @Test
    public void testWaitUntil_InterruptEx() throws InterruptedException {
        multi = new TestTimeMulti();
        ...
        Thread thread = new Thread() {
            ...
            public void run() {
                try {
                    multi.waitUntil(DELAY_MS, TimeUnit.MILLISECONDS, callable);
                } catch (Error ex) {
                    errors.add(ex);
                }
            }
    ...
    @Test
    public void testWaitUntil_ConditionThrowsEx() throws InterruptedException {
        multi = new TestTimeMulti();
        ...
        final long realBegin = System.currentTimeMillis();
        assertThatThrownBy(() -> multi.waitUntil(DELAY_MS, TimeUnit.MILLISECONDS, callable))
    ...
    @Test
    public void testWaitUntil_PseudoTimeExpires() throws InterruptedException {
        multi.enqueue(new WorkItem(multi, DELAY_MS));
        multi.enqueue(new WorkItem(multi, DELAY_MS * 2));
        multi.enqueue(new WorkItem(multi, DELAY_MS * 3));
        ...
        assertThatThrownBy(() -> multi.waitUntil(DELAY_MS * 2 - 1, TimeUnit.MILLISECONDS, () -> false))
                        .hasMessage("condition was never satisfied");
        assertEquals(tbegin + DELAY_MS * 2, multi.getMillis());
    }

    @Test
    public void testWaitUntil_RealTimeExpires() throws InterruptedException {
        multi = new TestTimeMulti(SHORT_WAIT_MS);

        final long realBegin = System.currentTimeMillis();
        assertThatThrownBy(() -> multi.waitUntil(SHORT_WAIT_MS, TimeUnit.MILLISECONDS, () -> false))
                        .hasMessage("condition was never satisfied");
        assertTrue(System.currentTimeMillis() >= realBegin + SHORT_WAIT_MS);
    }
