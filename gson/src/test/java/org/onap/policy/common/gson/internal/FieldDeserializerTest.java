...
public class FieldDeserializerTest {
    ...
    public void testGetFromTree() throws Exception {
        deser = new FieldDeserializer(gson, FieldDeserializerTest.class.getDeclaredField("text"));

        JsonObject json = new JsonObject();

        // no value in tree - text remains the unchanged
        text = INITIAL_VALUE;
        deser.getFromTree(json, this);
        assertEquals(text, INITIAL_VALUE);

        // null value in tree - text remains the unchanged
         ...
         * check list field
         */
        deser = new FieldDeserializer(gson, FieldDeserializerTest.class.getDeclaredField("listField"));
        
        json.add("listField", DataAdapterFactory.makeArray());
