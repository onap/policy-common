...
public class StateManagement extends Observable {
    ...
    public StateManagement(final EntityManagerFactory entityManagerFactory, final String resourceName)
        ...
        try (final EntityMgrCloser emc = new EntityMgrCloser(em); final MyTransaction et = new MyTransaction(em)) {
    ...
    public String getStandbyStatus(final String otherResourceName) {
        ...
        try (final EntityMgrCloser emc = new EntityMgrCloser(em); final MyTransaction et = new MyTransaction(em)) {
            ...
            final Query stateManagementListQuery = em.createQuery(GET_STATE_MANAGEMENT_ENTITY_QUERY);
            stateManagementListQuery.setParameter(RESOURCE_NAME, otherResourceName);
            final List<?> stateManagementList = stateManagementListQuery.setLockMode(LockModeType.NONE)
                    .setFlushMode(FlushModeType.COMMIT).getResultList();
            if (stateManagementList.size() == 1 && stateManagementList.get(0) instanceof StateManagementEntity) {
                final StateManagementEntity stateManagementEntity = (StateManagementEntity) stateManagementList.get(0);
                // refresh the object from DB in case cached data was returned
                em.refresh(stateManagementEntity);
    ...
    public void deleteAllStateManagementEntities() {
        ...
        try (final EntityMgrCloser emc = new EntityMgrCloser(em); final MyTransaction et = new MyTransaction(em)) {
            final Query stateManagementEntityListQuery = em.createQuery("SELECT p FROM StateManagementEntity p");
            ...
            for (final StateManagementEntity stateManagementEntity : stateManagementEntityList) {
                if (LOGGER.isDebugEnabled()) {
                    LOGGER.debug(
                            "deleteAllStateManagementEntities: Deleting statemanagemententity with resourceName={} and standbyStatus={}",
