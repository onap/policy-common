...
public class HttpEndpointPropertiesHelperTest {
    ...
    @Test
    public void test() {
        Assert.assertTrue("http.A.services".equals(propHelper.getEndpointPropertyName()));

        propHelper.setEndpointNames(Arrays.asList("M"));
        Assert.assertTrue(propHelper.getEndpointNames().size() == 1);
        Assert.assertTrue("M".equals(propHelper.getEndpointNames().get(0)));

        propHelper.setEndpointNames("X,Y");
        Assert.assertTrue(propHelper.getEndpointNames().size() == 2);
        Assert.assertTrue("X".equals(propHelper.getEndpointNames().get(0)));
        Assert.assertTrue("Y".equals(propHelper.getEndpointNames().get(1)));

        propHelper.setHost("X", "host");
        Assert.assertTrue("host".equals(propHelper.getHost("X")));

        propHelper.setPort("X", 7777);
        Assert.assertTrue(7777 == propHelper.getPort("X"));

        propHelper.setUserName("X", "username");
        Assert.assertTrue("username".equals(propHelper.getUserName("X")));

        propHelper.setPassword("X", "password");
        Assert.assertTrue("password".equals(propHelper.getPassword("X")));

        propHelper.setContextUriPath("Y", "context");
        Assert.assertFalse("context".equals(propHelper.getContextUriPath("X")));
        Assert.assertTrue("context".equals(propHelper.getContextUriPath("Y")));
        ...
        Assert.assertTrue("true".equals(propHelper.getHttps("X")));
        ...
        Assert.assertTrue("true".equals(propHelper.getManaged("X")));
    }
}