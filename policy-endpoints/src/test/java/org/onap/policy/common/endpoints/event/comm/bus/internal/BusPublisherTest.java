...
public class BusPublisherTest extends BusTopicTestBase {
    ...
    private static final String PASSWD = "my-pass";
    private static final String USERNAME = "my-user";
    private static final String PARTITION = "my-partition";
    ...
    public void testCambriaPublisherWrapperSend() throws Exception {
        ...
        assertTrue(cambria.send("send-part-1", "send-msg-1"));

        // publisher exception
        when(pub.send(anyString(), anyString())).thenThrow(new IOException(EXPECTED));
        assertFalse(cambria.send("send-part-2", "send-msg-2"));
    }
    ...
    public void testCambriaPublisherWrapperSend_InvalidMsg() {
        ...
        cambria.send("send-part-invalid", null);
    }
    ...
    public void testDmaapPublisherWrapper() {
        ...
        new DmaapAafPublisherWrapper(servers, TOPIC, USERNAME, PASSWD, true);
        new DmaapAafPublisherWrapper(servers, TOPIC, USERNAME, PASSWD, false);
        new DmaapPublisherWrapper(ProtocolTypeConstants.DME2, servers, TOPIC, USERNAME, PASSWD, true) {};
    }

    @Test(expected = IllegalArgumentException.class)
    public void testDmaapPublisherWrapper_InvalidTopic() {
        new DmaapPublisherWrapper(ProtocolTypeConstants.DME2, servers, "", USERNAME, PASSWD, true) {};
    }

    @Test(expected = IllegalArgumentException.class)
    public void testDmaapPublisherWrapper_Aaf_NullServers() {
        new DmaapAafPublisherWrapper(null, TOPIC, USERNAME, PASSWD, true);
    }

    @Test(expected = IllegalArgumentException.class)
    public void testDmaapPublisherWrapper_Aaf_NoServers() {
        new DmaapAafPublisherWrapper(Collections.emptyList(), TOPIC, USERNAME, PASSWD, true);
    }

    @Test(expected = IllegalArgumentException.class)
    public void testDmaapPublisherWrapper_InvalidProtocol() {
        new DmaapPublisherWrapper(ProtocolTypeConstants.HTTPNOAUTH, servers, TOPIC, USERNAME, PASSWD, true) {};
    }
    ...
    public void testDmaapPublisherWrapperClose() throws Exception {
        ...
        DmaapPublisherWrapper dmaap = new DmaapAafPublisherWrapper(servers, TOPIC, USERNAME, PASSWD, true);
        dmaap.publisher = pub;
    ...
    public void testDmaapPublisherWrapperSend() throws Exception {
        ...
        // null response
        assertTrue(dmaap.send(PARTITION, MESSAGE));
        verify(pub).setPubResponse(any(MRPublisherResponse.class));
        verify(pub).send(PARTITION, MESSAGE);
    ...
    public void testDmaapDmePublisherWrapper() {
        ...
        addProps.put("routeOffer", "my-route");
        new DmaapDmePublisherWrapper(makeBuilder().build());
