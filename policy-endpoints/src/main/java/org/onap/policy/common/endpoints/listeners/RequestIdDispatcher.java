...
public class RequestIdDispatcher<T> extends ScoListener<T> {
     ...
     * @param requestIdFieldNames name of the request id field, which may be a hierarchy
     */
    ...
    public void onTopicEvent(CommInfrastructure infra, String topic, StandardCoderObject sco, T message) {
        ...
        if (Strings.isNullOrEmpty(reqid)) {
            ...
            for (TypedMessageListener<T> listener : listeners) {
                offerToHandler(infra, topic, message, listener);
            }

        } else {
            // it's a response to a particular request
            offerToHandler(infra, topic, message, req2listener.get(reqid));
        }
    }
     ...
     */
    private void offerToHandler(CommInfrastructure infra, String topic, T msg, TypedMessageListener<T> listener) {
