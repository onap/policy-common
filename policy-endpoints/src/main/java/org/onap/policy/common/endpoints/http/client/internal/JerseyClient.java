...
public class JerseyClient implements HttpClient {
    ...
    public JerseyClient(BusTopicParams busTopicParams) throws KeyManagementException, NoSuchAlgorithmException {
        ...
        if (busTopicParams.getClientName() == null || busTopicParams.getClientName().isEmpty()) {
            throw new IllegalArgumentException("Name must be provided");
        }

        if (busTopicParams.getHostname() == null || busTopicParams.getHostname().isEmpty()) {
            throw new IllegalArgumentException("Hostname must be provided");
        }

        if (busTopicParams.getPort() <= 0 && busTopicParams.getPort() >= 65535) {
