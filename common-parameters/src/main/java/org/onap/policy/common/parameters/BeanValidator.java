...
public class BeanValidator {
     ...
     * fields to be verified, but it does not examine any interfaces. If it finds an
     * annotated field within a class, then it only searches for a public "getter" method
     * down the hierarchy from that class.
     *
    ...
    private void validateField(BeanValidationResult result, Object object, Class<?> clazz, Field field) {
        ...
        Method accessor = getAccessor(clazz, fieldName);
        if (accessor == null) {
    ...
    /**
     * Looks for an annotation at the class or field level.
     *
    ...
    private boolean verMin(BeanValidationResult result, String fieldName, Min annot, Object value) {
        ...
        ObjectValidationResult fieldResult = new ObjectValidationResult(fieldName, value, ValidationStatus.INVALID,
                        "exceeds the minimum value: " + annot.value());
        result.addResult(fieldResult);
        return false;
    }
