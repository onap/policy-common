...
public class BeanValidator {
    ...
    public boolean verSize(BeanValidationResult result, String fieldName, Size annot, Object value) {
        if (value instanceof Collection) {
            if (((Collection<?>) value).size() < annot.min()) {
                result.addResult(fieldName, xlate(value),
                                ValidationStatus.INVALID, "minimum number of elements: " + annot.min());
                return false;
            }
