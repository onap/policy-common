...
public class TestBeanValidator2 {
    ...
    public void testValidate() {
        ...
        // @formatter:off
        assertThat(validator.validate(makeUser(OTHER_NAME, MY_AGE, false, null, false)))
            .contains("address", "state", "must not be null")
            .contains("checks", "item 0", "isOk", "must return true")
            .contains("name", "Tom", "must match")
            .contains("sleeping", "false", "must return true");
        // @formatter:on
        ...
        assertThat(validator.validate(user))
            .contains("checks", "item 0", "must not be null")
            .contains("type2check", "<map key>", "must not be null")
            .contains("type2check", "<map value>", "must not be null");
        // @formatter:on
    }
    ...
    public void testAddValue() {
        ...
        assertThat(validator.validate(new User(MY_NAME, MY_AGE, true, null, null, null)))
                        .contains("address", "null", "must not be null");
        ...
        assertThat(validator.validate(makeUser(MY_NAME, OTHER_AGE, true, MY_STATE, true)))
                        .contains("age", "200", "must be less than or equal to 20");

        // boxed primitive
        assertThat(validator.validate(makeUser(MY_NAME, MY_AGE, false, MY_STATE, true)))
                        .contains("sleeping", "false", "must be true");
