 ...
 * left out of the {@link Properties}. Consequently, if the <i>useDefault</i> option is
 * {@code true}, then the <i>defaultValue</i> will be validated, even if it is not used.
 */
public class PropertyConfiguration {
    ...
    protected Object getValue(Field field, Properties props, Property prop) throws PropertyException {
        ...
        // TODO add short, float, double, enum
    ...
    protected Boolean getBooleanValue(Field field, Properties props, Property prop) throws PropertyException {
        if (prop.useDefault()) {
            ...
            checkDefaultValue(field, xxx -> makeBoolean(field, prop, prop.defaultValue()));
        }
    ...
    protected Integer getIntegerValue(Field field, Properties props, Property prop) throws PropertyException {
        if (prop.useDefault()) {
            // validate the default value
            checkDefaultValue(field, xxx -> makeInteger(field, prop, prop.defaultValue()));
        }
    ...
    protected Long getLongValue(Field field, Properties props, Property prop) throws PropertyException {
        if (prop.useDefault()) {
            // validate the default value
            checkDefaultValue(field, xxx -> makeLong(field, prop, prop.defaultValue()));
        }
     ...
     *         value does not exist and <i>useDefault</i> is true
     * @throws PropertyMissingException if the property does not exist or is {@code null},
     *         and <i>useDefault</i> is {@code false}
     */
     ...
     */
    private void checkDefaultValue(Field field, CheckDefaultValueFunction func) throws PropertyInvalidException {
        try {
            func.apply(null);

        } catch (PropertyInvalidException ex) {
            throw new PropertyInvalidException(ex.getPropertyName(), field, "defaultValue is invalid", ex.getCause());
        }
    }
    ...
    protected static @interface Property {
        ...
        /**
         * Indicates whether or not the default value should be used for a missing
         * property.
         * 
         * @return {@code true} if the default value should be used for a missing
         *         property, {@code false} otherwise.
         */
        public boolean useDefault() default false;
