...
public class PropertyCoder {
    ...
    public <T> T decode(Reader reader, Class<T> clazz) throws GeneralSecurityException,
        CoderException {
    ...
    private static class MyDecoder extends StandardCoder {
        ...
        MyDecoder(JsonElement jsonElement, String key) throws CoderException {
            ...
            // Use key from jsonObject first, else use one from input
            String secretKey = jsonObject.get(AES_ENCRYPTION_KEY).getAsString();
            if (secretKey != null) {
                crypto = new CryptoUtils(secretKey);
                if (crypto == null) {
                    throw new CoderException("WARNING: PropertyUtil - CryptoUtils is null");
                }
                return;
            }
