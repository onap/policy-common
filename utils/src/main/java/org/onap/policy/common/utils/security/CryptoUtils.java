...
public class CryptoUtils {
    ...
    private static final byte[] iv = new byte[IV_BLOCK_SIZE_IN_BYTES];
     ...
     * Final result is to put in properties file is: IV + Outcome of openssl command
     *
     * @param value
    ...
    private static String decryptValue(String value, SecretKeySpec keySpec) {
        ...
        if (!isEncrypted(value)) {
            return value;
        } else if (value.length() < validSize) {
     ...
     * @return boolean value indicate if string prefix with end: or not
     */
