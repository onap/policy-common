...
public class CommandLineArgumentsHandler {
    ...
    protected CommandLineArgumentsHandler(String helpClassName, String component) {
        ...
        options.addOption(Option.builder("v")
                .longOpt("version")
                .desc(String.format("outputs the version of %s", this.component))
                .required(false)
                .type(Boolean.class)
                .build());
    ...
    public void validate() throws CommandLineException {
        validateReadableFile(String.format("%s configuration", this.component), configurationFilePath);
    }
    ...
    public String help() {
        ...
        helpFormatter.printHelp(printWriter, HELP_LINE_LENGTH, helpClassName + " [options...]", "options", options, 0,
                0, "");
    ...
    public boolean checkSetConfigurationFilePath() {
        return getConfigurationFilePath() != null && getConfigurationFilePath().trim().length() > 0;
    }
    ...
    public boolean checkSetPropertyFilePath() {
        return getPropertyFilePath() != null && getPropertyFilePath().trim().length() > 0;
    }
    ...
    private String[] removeEmptyValues(String[] args) {
        return Arrays.stream(args).filter(s -> (s.trim().length() > 0)).toArray(String[]::new);
    }
